...
import { ListarCategorias } from '../functions/CategoriasFunctions';
import { ListarEmpaques } from '../functions/EmpaquesFunctions';
import { ListarUbicaciones } from '../functions/UbicacionFuncitons';
import { ListarElementos } from '../functions/ElementosFunctions';
...
const Elementos = () => {
  ...
  const fetchData = async() => {
    try {
      ...
      const categoriasInfo = responseCategorias.categorias;
      setCategorias(categoriasInfo);

      const responseEmpaques = await ListarEmpaques();
      const empaquesInfo = responseEmpaques.empaques;
      SetEmpaques(empaquesInfo);

      const responseUbicaciones = await ListarUbicaciones();
      const ubicacionesInfo = responseUbicaciones.ubicaciones;
      SetUbicacion(ubicacionesInfo);

      const responseElementos = await ListarElementos();
      const elementosInfo = responseElementos.elementos;
      setElementos(elementosInfo);
      
      setColumnsHeader(Object.keys(elementosInfo[0]))
    } catch (error) {
  ...
  return (
    <>
      ...
      <div>
        ...
        <Table2 columns={columnsHeader} data={elementos} />
      </div>
    </>
  )
}
